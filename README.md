<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <title>Calculadora Avançada de Despesas Imobiliárias</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      max-width: 800px;
      margin: 20px auto;
      padding: 20px;
      background-color: #f4f4f4;
      border-radius: 8px;
      box-shadow: 0 0 10px #ccc;
    }
    h2 { text-align: center; }
    label { display: block; margin-top: 12px; }
    input { width: 100%; padding: 8px; margin-top: 4px; box-sizing: border-box; }
    button {
      margin-top: 20px;
      padding: 10px;
      width: 100%;
      background-color: #007bff;
      color: white;
      border: none;
      border-radius: 4px;
      font-size: 16px;
    }
    .resultado {
      margin-top: 20px;
      padding: 15px;
      background: #fff;
      border: 1px solid #ccc;
      border-radius: 5px;
    }
    hr { margin: 15px 0; }
  </style>
</head>
<body>

<h2>Calculadora Avançada de Despesas</h2>

<form id="formulario">
  <h3>Aluguel</h3>
  <label>Valor mensal do aluguel (R$): <input type="number" step="0.01" id="aluguelMensal" required></label>
  <label>Data início: <input type="date" id="dataInicioAluguel" required></label>
  <label>Data fim: <input type="date" id="dataFimAluguel" required></label>

  <h3>IPTU</h3>
  <label>Valor da parcela (R$): <input type="number" step="0.01" id="iptuParcela"></label>
  <label>Quantidade de parcelas: <input type="number" id="iptuQtdParcelas"></label>
  <label>Total do IPTU (R$): <input type="number" id="iptuTotal" readonly></label>
  <label>Valor já pago (R$): <input type="number" step="0.01" id="iptuPago"></label>
  <label>Data início uso IPTU: <input type="date" id="iptuInicio" required></label>
  <label>Data fim uso IPTU: <input type="date" id="iptuFim" required></label>

  <h3>Seguro Incêndio</h3>
  <label>Valor total (R$): <input type="number" step="0.01" id="seguroTotal" required></label>
  <label>Valor pago (R$): <input type="number" step="0.01" id="seguroPago"></label>
  <label>Data início uso seguro: <input type="date" id="seguroInicio" required></label>
  <label>Data fim uso seguro: <input type="date" id="seguroFim" required></label>

  <h3>Pintura</h3>
  <label>Valor total (R$): <input type="number" step="0.01" id="pinturaTotal"></label>
  <label>Valor pago (R$): <input type="number" step="0.01" id="pinturaPago"></label>

  <h3>Manutenção</h3>
  <label>Valor estimado (R$): <input type="number" step="0.01" id="manutencao"></label>

  <h3>Água</h3>
  <label>Valor total (R$): <input type="number" step="0.01" id="aguaTotal"></label>
  <label>Valor pago (R$): <input type="number" step="0.01" id="aguaPago"></label>

  <h3>Luz</h3>
  <label>Valor total (R$): <input type="number" step="0.01" id="luzTotal"></label>
  <label>Valor pago (R$): <input type="number" step="0.01" id="luzPago"></label>

  <h3>Condomínio</h3>
  <label>Valor total (R$): <input type="number" step="0.01" id="condominioTotal"></label>
  <label>Valor pago (R$): <input type="number" step="0.01" id="condominioPago"></label>

  <h3>Gás</h3>
  <label>Valor total (R$): <input type="number" step="0.01" id="gasTotal"></label>
  <label>Valor pago (R$): <input type="number" step="0.01" id="gasPago"></label>

  <button type="submit">Calcular</button>
</form>

<div class="resultado" id="resultado" style="display: none;"></div>

<script>
  const formatar = valor => new Intl.NumberFormat('pt-BR', {
    style: 'currency',
    currency: 'BRL'
  }).format(valor);

  const diasEntreDatas = (inicio, fim) => {
    const i = new Date(inicio);
    const f = new Date(fim);
    return Math.floor((f - i) / (1000 * 60 * 60 * 24)) + 1;
  };

  const lerNumero = id => parseFloat(document.getElementById(id).value.replace(',', '.')) || 0;

  // Atualiza total do IPTU automaticamente
  const iptuParcelaInput = document.getElementById('iptuParcela');
  const iptuQtdParcelasInput = document.getElementById('iptuQtdParcelas');
  const iptuTotalInput = document.getElementById('iptuTotal');

  function atualizarIptuTotal() {
    const parcela = lerNumero('iptuParcela');
    const qtd = lerNumero('iptuQtdParcelas');
    iptuTotalInput.value = parcela * qtd;
  }

  iptuParcelaInput.addEventListener('input', atualizarIptuTotal);
  iptuQtdParcelasInput.addEventListener('input', atualizarIptuTotal);

  document.getElementById('formulario').addEventListener('submit', function(e) {
    e.preventDefault();
    let resumo = "";

    // Aluguel pró-rata
    const aluguelMensal = lerNumero('aluguelMensal');
    const aluguelDias = diasEntreDatas(
      document.getElementById('dataInicioAluguel').value,
      document.getElementById('dataFimAluguel').value
    );
    const aluguelDiario = aluguelMensal / 30;
    const aluguelTotal = aluguelDiario * aluguelDias;
    resumo += `<p><strong>Aluguel pró-rata (${aluguelDias} dias):</strong> ${formatar(aluguelTotal)}</p>`;

    // IPTU proporcional
    const iptuTotal = lerNumero('iptuTotal');
    const iptuPago = lerNumero('iptuPago');
    if (iptuTotal > 0) {
      const iptuDias = diasEntreDatas(
        document.getElementById('iptuInicio').value,
        document.getElementById('iptuFim').value
      );
      const iptuProporcional = (iptuTotal / 360) * iptuDias;
      const iptuDiferenca = iptuProporcional - iptuPago;
      if (iptuPago !== 0 || iptuDiferenca !== 0) {
        resumo += `<p><strong>IPTU proporcional (${iptuDias} dias):</strong> ${formatar(iptuProporcional)} - Pago: ${formatar(iptuPago)} 
        → ${iptuDiferenca >= 0 ? "A pagar: " + formatar(iptuDiferenca) : "A devolver: " + formatar(Math.abs(iptuDiferenca))}</p>`;
      }
    }

    // Seguro incêndio
    const seguroTotal = lerNumero('seguroTotal');
    const seguroPago = lerNumero('seguroPago');
    if (seguroTotal > 0) {
      const seguroDias = diasEntreDatas(
        document.getElementById('seguroInicio').value,
        document.getElementById('seguroFim').value
      );
      const seguroProporcional = (seguroTotal / 360) * seguroDias;
      const seguroDiferenca = seguroPago - seguroProporcional;
      if (seguroPago !== 0 || seguroDiferenca !== 0) {
        resumo += `<p><strong>Seguro incêndio (${seguroDias} dias):</strong> ${formatar(seguroProporcional)} - Pago: ${formatar(seguroPago)} 
        → ${seguroDiferenca >= 0 ? "A devolver: " + formatar(seguroDiferenca) : "A pagar: " + formatar(Math.abs(seguroDiferenca))}</p>`;
      }
    }

    // Pintura
    const pinturaTotal = lerNumero('pinturaTotal');
    const pinturaPago = lerNumero('pinturaPago');
    const pinturaDiferenca = pinturaTotal - pinturaPago;
    if (pinturaTotal > 0 && (pinturaPago !== 0 || pinturaDiferenca !== 0)) {
      resumo += `<p><strong>Pintura:</strong> ${formatar(pinturaTotal)} - Pago: ${formatar(pinturaPago)} → A pagar: ${formatar(pinturaDiferenca)}</p>`;
    }

    // Manutenção
    const manutencaoBase = lerNumero('manutencao');
    if (manutencaoBase > 0) {
      const manutencaoTotal = manutencaoBase * 1.2;
      resumo += `<p><strong>Manutenção (com 20%):</strong> ${formatar(manutencaoTotal)}</p>`;
    }

    // Água
    const aguaTotal = lerNumero('aguaTotal');
    const aguaPago = lerNumero('aguaPago');
    if (aguaTotal > 0 && (aguaPago !== 0 || aguaTotal !== 0)) {
      resumo += `<p><strong>Água:</strong> ${formatar(aguaTotal)} - Pago: ${formatar(aguaPago)} → A pagar: ${formatar(aguaTotal - aguaPago)}</p>`;
    }

    // Luz
    const luzTotal = lerNumero('luzTotal');
    const luzPago = lerNumero('luzPago');
    if (luzTotal > 0 && (luzPago !== 0 || luzTotal !== 0)) {
      resumo += `<p><strong>Luz:</strong> ${formatar(luzTotal)} - Pago: ${formatar(luzPago)} → A pagar: ${formatar(luzTotal - luzPago)}</p>`;
    }

    // Condomínio
    const condominioTotal = lerNumero('condominioTotal');
    const condominioPago = lerNumero('condominioPago');
    if (condominioTotal > 0 && (condominioPago !== 0 || condominioTotal !== 0)) {
      resumo += `<p><strong>Condomínio:</strong> ${formatar(condominioTotal)} - Pago: ${formatar(condominioPago)} → A pagar: ${formatar(condominioTotal - condominioPago)}</p>`;
    }

    // Gás
    const gasTotal = lerNumero('gasTotal');
    const gasPago = lerNumero('gasPago');
    if (gasTotal > 0 && (gasPago !== 0 || gasTotal !== 0)) {
      resumo += `<p><strong>Gás:</strong> ${formatar(gasTotal)} - Pago: ${formatar(gasPago)} → A pagar: ${formatar(gasTotal - gasPago)}</p>`;
    }

    const resultado = document.getElementById('resultado');
    resultado.innerHTML = `<h3>Resumo:</h3>${resumo}`;
    resultado.style.display = 'block';
  });
</script>

</body>
</html>
